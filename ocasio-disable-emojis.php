<?php
/**
 * Plugin Name: Ocasio Disable Emojis
 * Plugin URI:  https://kevinocasio.com/wordpress-plugins/ocasio-disable-emojis/
 * Description: Removes extra emoji scripts, inline styles, and DNS prefetch links to speed up page load times.
 * Version:     1.0.0
 * Author:      Kevin Ocasio
 * Author URI:  https://kevinocasio.com
 * License:     GPL-2.0-or-later
 * License URI: https://www.gnu.org/licenses/gpl-2.0.html
 * Text Domain: ocasio-disable-emojis
 */
...
define('OCASIO_DE_VERSION', '1.0.0');
define('OCASIO_DE_PLUGIN_DIR', plugin_dir_path(__FILE__));
define('OCASIO_DE_PLUGIN_URL', plugin_dir_url(__FILE__));

/**
 * Helper: Smart Brand URL Resolver (Keeps links local on .local, points to live domain on production)
 */
function ocasio_de_brand_url($path = '/wordpress-plugins/') {
 ...
 */
function ocasio_de_activate() {
	if (get_option('ocasio_de_enabled') === false) {
		$legacy = get_option('ko_de_enabled', 1);
		update_option('ocasio_de_enabled', (int) $legacy);
	}
}
register_activation_hook(__FILE__, 'ocasio_de_activate');

/**
 * 2. Core Logic: Dequeue WordPress Emoji Scripts and Styles
 */
function ocasio_de_is_active() {
	$enabled = get_option('ocasio_de_enabled');
	if ($enabled === false) {
		$enabled = get_option('ko_de_enabled', 1);
	}
	return (int) $enabled === 1;
}

function ocasio_de_apply_optimization() {
	if (!ocasio_de_is_active()) {
		return;
	}
	...
	// Remove emoji filters from feeds and emails
	remove_filter('the_content_feed', 'wp_staticize_emoji');
	remove_filter('comment_text_rss', 'wp_staticize_emoji');
	remove_filter('wp_mail', 'wp_staticize_emoji_for_email');

	// Remove from TinyMCE editor
	add_filter('tiny_mce_plugins', 'ocasio_de_remove_tinymce_emoji');

	// Remove DNS prefetch for s.w.org
	add_filter('emoji_svg_url', '__return_false');
}
add_action('init', 'ocasio_de_apply_optimization');

function ocasio_de_remove_tinymce_emoji($plugins) {
	if (is_array($plugins)) {
 ...
 */
function ocasio_de_register_settings() {
	register_setting('ocasio_de_settings_group', 'ocasio_de_enabled', array(
		'type'              => 'integer',
		'sanitize_callback' => 'absint',
		'default'           => 1,
	));
}
add_action('admin_init', 'ocasio_de_register_settings');

/**
 * 4. Register Admin Menu under Ocasio Plugins (Position 65)
 */
function ocasio_de_register_menu() {
...
add_action('admin_menu', 'ocasio_de_register_menu');

/**
 * 5. Action Links on Plugins Screen
 */
function ocasio_de_action_links($links) {
	$settings_link = '<a href="' . esc_url(admin_url('admin.php?page=ocasio-plugins-main')) . '">' . esc_html__('Dashboard', 'ocasio-disable-emojis') . '</a>';
	array_unshift($links, $settings_link);
	return $links;
}
add_filter('plugin_action_links_' . plugin_basename(__FILE__), 'ocasio_de_action_links');

/**
 * 6. Conditional Admin Asset Enqueuing
 */
function ocasio_de_admin_assets($hook) {
	...
	if (strpos($hook, 'ocasio-plugins-main') !== false || $hook === 'toplevel_page_ocasio-plugins-main') {
		wp_enqueue_style('ocasio-de-admin-css', plugins_url('assets/admin.css', __FILE__), array(), OCASIO_DE_VERSION);
		wp_enqueue_script('ocasio-de-admin-js', plugins_url('assets/admin.js', __FILE__), array('jquery'), OCASIO_DE_VERSION, true);
		wp_localize_script('ocasio-de-admin-js', 'ocasio_vars', array(
			'ajaxurl'     => admin_url('admin-ajax.php'),
			'suite_nonce' => wp_create_nonce('ocasio_suite_toggle_nonce'),
		));
		wp_localize_script('ocasio-de-admin-js', 'ocasio_301_vars', array(
			'ajaxurl'     => admin_url('admin-ajax.php'),
			'suite_nonce' => wp_create_nonce('ocasio_suite_toggle_nonce'),
		));
	}
}
add_action('admin_enqueue_scripts', 'ocasio_de_admin_assets');
